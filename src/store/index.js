import { createStore } from 'vuex';
import axios from 'axios';

const store = createStore({
	state: {
		link_winbox77: null,
		error: null,
		isFetching: false
	},
	mutations: {
		SET_LINK_WINBOX77(state, link) {
			state.link_winbox77 = link;
		},
		SET_ERROR(state, error) {
			state.error = error;
		},
		SET_FETCHING(state, isFetching) {
			state.isFetching = isFetching;
		}
	},
	actions: {
		async fetchLink_winbox77({ commit, state }) {
			if (!state.link_winbox77 && !state.isFetching) {
				// 防止重复调用
				commit('SET_FETCHING', true);  // 设置正在获取数据的状态
				try {
					const response = await axios.get(
						'https://seo.mobileapplab.online/api/winbox?fields[0]=winbox77_my',
						{
							headers: {
								Authorization: 'Bearer '
							}
						}
					);
					commit('SET_LINK_WINBOX77', response.data.data.attributes.winbox77_my);
				} catch (error) {
					console.error(error);
				} finally {
					commit('SET_FETCHING', false);  // 恢复状态
				}
			}
		}
	},
	getters: {
		link_winbox77: state => state.link_winbox77,
		error: state => state.error
	}
});

export default store;