import { createStore } from 'vuex';
import axios from 'axios';

const store = createStore({
	state: {
		link_winbox: null,
		error: null,
		isFetching: false
	},
	mutations: {
		SET_LINK_WINBOX(state, link) {
			state.link_winbox = link;
		},
		SET_ERROR(state, error) {
			state.error = error;
		},
		SET_FETCHING(state, isFetching) {
			state.isFetching = isFetching;
		}
	},
	actions: {
		async fetchLink_winbox({ commit, state }) {
			if (!state.link_winbox && !state.isFetching) {
				// 防止重复调用
				commit('SET_FETCHING', true);  // 设置正在获取数据的状态
				try {
					const response = await axios.get(
						'https://seo.mobileapplab.online/api/winbox?fields[0]=winboxofficial_my',
						{
							headers: {
								Authorization: 'Bearer '
							}
						}
					);
					commit('SET_LINK_WINBOX', response.data.data.attributes.winboxofficial_my);
				} catch (error) {
					console.error(error);
				} finally {
					commit('SET_FETCHING', false);  // 恢复状态
				}
			}
		}
	},
	getters: {
		link_winbox: state => state.link_winbox,
		error: state => state.error
	}
});

export default store;